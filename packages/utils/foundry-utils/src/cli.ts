...
program
    ...
    .action(async (props) => {
        ...
        const nameWithoutSuffix = name.endsWith('Tool') ? name.slice(0, -4) : name
...
\`\`\`
        ...
        fs.writeFileSync(path.join(root, 'docs', 'tools', type, `${nameWithoutSuffix}.mdx`), docs)

        const mintConfig = JSON.parse(fs.readFileSync(path.join(root, 'docs', 'mint.json'), 'utf8'))
        ...
        fs.writeFileSync(path.join(root, 'docs', 'mint.json'), JSON.stringify(mintConfig, null, 4))
    })
